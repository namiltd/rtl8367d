				...
				bool permit_pause_to_mac)
{
	...
	if (is_d) {
		 ...
		 * MAC6_SEL_SDS0 and the mode), and comes out as 0x0E12 - the
		 * same value the stock firmware holds with the trunk up.
		 *
		 * The receiver latches on a DISABLE -> HSGMII edge rather
		 * than on the value, but that edge cannot be driven from
		 * here - it's driven later by the relatch workqueue, once
		 * the far end has a chance of being up.
		 */
