 ...
 * the RTL8367S and RTL8367SB, the SGMII and HSGMII interfaces are also supported.
 *
...
#define   RTL8365MB_SDS_INDACS_CMD_MASK		0x0080
#define   RTL8365MB_SDS_INDACS_RWO_MASK		0x0040
...
/* Write data and address, then trigger the indirect access command.
 * The SerDes requires data before address before command trigger.
 */
static int rtl8365mb_sds_indacs_write(struct realtek_priv *priv, u16 addr,
				       u16 data)
{
	int ret;
	...
	return regmap_write(priv->map, RTL8365MB_SDS_INDACS_CMD_REG,
			    RTL8365MB_SDS_INDACS_CMD_MASK |
			    RTL8365MB_SDS_INDACS_RWO_MASK);
}

/* Poll until the previous indirect access command completes.
 * The BUSY bit is set by hardware while the command is executing.
 */
				      ...
				      phy_interface_t interface)
{
	...
	for (i = 0; i < sds_jam_size; i++) {
		ret = rtl8365mb_sds_indacs_write(priv,
						 sds_jam[i].reg,
						 sds_jam[i].val);
		if (ret)
			return ret;
	...
	ret = rtl8365mb_sds_indacs_write(priv, RTL8365MB_SDS_REG_RESET,
				   RTL8365MB_SDS_RESET_DEASSERT);
	if (ret)
		return ret;

	ret = rtl8365mb_sds_indacs_write(priv, RTL8365MB_SDS_REG_BMCR,
	...
	/* Disable NWAY / autonegotiation on the SerDes.
	 * SGMII in-band autonegotiation is not supported; link parameters
	 * are forced from mac_link_up/down via SDS_MISC.
	 */
	ret = rtl8365mb_sds_indacs_read(priv, RTL8365MB_SDS_REG_NWAY, &val);
	if (ret)
		return ret;
	...
	return rtl8365mb_sds_indacs_write(priv, RTL8365MB_SDS_REG_NWAY, val);
}

/* Check if the interface mode uses the embedded SerDes */
				       ...
				       struct phylink_config *config)
{
	...
	/* HSGMII is represented by phylink as 2500base-x */
					 ...
					 const struct phylink_link_state *state)
{
	...
	if (rtl8365mb_interface_is_serdes(state->interface)) {
		...
		if (ret)
			dev_err(priv->dev,
				"failed to configure SGMII or HSGMII mode on port %d: %pe\n",
				port, ERR_PTR(ret));
		return;
	}
					    ...
					    phy_interface_t interface)
{
	...
	if (phy_interface_mode_is_rgmii(interface) ||
		...
		ret = rtl8365mb_ext_config_forcemode(priv, port, interface, false, 0, 0,
						     false, false);
		if (ret)
			dev_err(priv->dev,
					  ...
					  bool rx_pause)
{
	...
	if (phy_interface_mode_is_rgmii(interface) ||
	    rtl8365mb_interface_is_serdes(interface)) {
		ret = rtl8365mb_ext_config_forcemode(priv, port, interface, true, speed,
						     duplex, tx_pause,
						     rx_pause);
		if (ret)
			dev_err(priv->dev,
