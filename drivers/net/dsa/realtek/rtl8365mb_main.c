				...
				bool permit_pause_to_mac)
{
	 ...
	 */
	if (is_d) {
		u32 park = (misc_val & ~RTL8365MB_D_SDS_MISC_MODE_FIELD_MASK) |
			   RTL8365MB_D_PORT_SDS_MODE_DISABLE;
		u32 current_reg;

		/* Store the target value for the relatch workqueue before parking */
		...
		ret = regmap_read(priv->map, RTL8365MB_SDS_MISC_REG, &current_reg);
		if (ret) {
			mutex_unlock(&mb->sds_lock);
			return ret;
		}

		if ((current_reg & RTL8365MB_D_SDS_MISC_MODE_FIELD_MASK) !=
		    RTL8365MB_D_PORT_SDS_MODE_DISABLE)
			dev_warn(priv->dev, "SerDes mode changed unexpectedly during park sleep\n");
...
static void rtl8365mb_sds_relatch_work(struct work_struct *work)
{
	...
	if ((target & RTL8365MB_D_SDS_MISC_MODE_FIELD_MASK) ==
	    RTL8365MB_D_PORT_SDS_MODE_DISABLE) {
		dev_warn(priv->dev,
			 "SDS_MISC parked at DISABLE with no recoverable target mode, giving up\n");
		return;
	}
